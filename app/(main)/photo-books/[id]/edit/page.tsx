...
export default async function EditPhotoBookPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="mx-auto max-w-6xl pb-8"><Link href="/photo-books" className="flex items-center gap-2 text-sm text-[#0b6254]"><ArrowLeft size={16}/>Kembali ke Photo Book</Link><h1 className="mt-4 font-serif text-4xl">Susun Photo Book</h1><p className="mb-5 mt-1 text-sm text-[#617078]">{book.title} · {book.status} · {pages.length} halaman</p><PhotoBookEditor book={book} saved={pages} available={available} action={savePhotoBookAction.bind(null, id)}/></div>;
}
