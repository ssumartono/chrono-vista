...
export function PhotoBookEditor({ book, available, saved, templates, action }: { book: Book; available: Photo[]; saved: Page[]; templates: TemplateOption[]; action: (form: FormData) => Promise<void> }) {
  ...
  const [currentTemplate, setCurrentTemplate] = useState(book.template);
  ...
  return <form action={action} className="space-y-5">
    ...
    <section className="grid gap-3 rounded border border-[#d9dfdc] bg-white p-5 sm:grid-cols-2">
      ...
      <label className="text-sm">Template<select name="template" value={currentTemplate} onChange={event => setCurrentTemplate(event.target.value)} className={inputClass}>{templates.map(template => <option key={template.value} value={template.value}>{template.label}</option>)}</select></label>
      <label className="text-sm">Ukuran Halaman<select name="pageSize" defaultValue={book.pageSize} disabled={currentTemplate.startsWith('custom:')} className={inputClass}><option>A4</option><option>A5</option><option>Square</option></select>{currentTemplate.startsWith('custom:') && <span className="mt-1 block text-xs text-[#617078]">Template buatan menggunakan spread A4.</span>}</label>
      <label className="text-sm">Margin (mm)<input name="marginMm" type="number" required min="0" max="50" defaultValue={book.marginMm} disabled={currentTemplate.startsWith('custom:')} className={inputClass}/>{currentTemplate.startsWith('custom:') && <span className="mt-1 block text-xs text-[#617078]">Atur posisi dan margin elemen di Kelola Template.</span>}</label>
